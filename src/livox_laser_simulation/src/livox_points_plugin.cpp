...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        // 设置 PointCloud2 字段描述
        scan_point.fields.resize(3);
        scan_point.fields[0].name = "x";
        scan_point.fields[0].offset = 0;
        scan_point.fields[0].datatype = sensor_msgs::PointField::FLOAT32;
        scan_point.fields[0].count = 1;

        scan_point.fields[1].name = "y";
        scan_point.fields[1].offset = 4;
        scan_point.fields[1].datatype = sensor_msgs::PointField::FLOAT32;
        scan_point.fields[1].count = 1;

        scan_point.fields[2].name = "z";
        scan_point.fields[2].offset = 8;
        scan_point.fields[2].datatype = sensor_msgs::PointField::FLOAT32;
        scan_point.fields[2].count = 1;

        scan_point.point_step = 12;
        scan_point.height = 1;
        scan_point.is_bigendian = false;
        scan_point.is_dense = true;

        // 预分配数据缓冲区
        size_t point_count = points_pair.size();
        scan_point.data.resize(point_count * 12);
        scan_point.width = point_count;
        scan_point.row_step = scan_point.data.size();

        float *data_ptr = reinterpret_cast<float *>(scan_point.data.data());
